...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!extractRes.ok) throw new Error("Falha ao extrair PDF");
    ...
    <main className="min-h-screen bg-neutral-50 text-neutral-900 p-4 md:p-8 font-sans">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <header className="text-center space-y-2">
          ...
          <p className="text-neutral-500">Adapte seu currículo para qualquer vaga usando Inteligência Artificial.</p>
        </header>
          ...
          >
            <Card>
              <CardHeader>
                ...
                <CardDescription>Faça upload do seu currículo base em formato PDF.</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <Label htmlFor="resume-upload" className="cursor-pointer flex flex-col items-center gap-2">
                    {file ? (
                      ...
                      <>
                        ...
                        <span className="font-medium text-neutral-600">Clique para enviar seu PDF</span>
                      </>
                    )}
                  </Label>
                </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Cole o texto completo da descrição da vaga (requisitos, diferenciais).</CardDescription>
              </CardHeader>
              <CardContent>
                <Textarea 
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm text-neutral-500">Escolha o idioma do currículo final</p>
                  </div>
          ...
          >
            <Card className="h-full">
              <CardHeader>
                ...
                <CardDescription>Seu currículo adaptado e carta de apresentação aparecerão aqui.</CardDescription>
              </CardHeader>
              <CardContent>
                {result ? (
                  <div className="space-y-6">
                    ...
                    <Tabs defaultValue="resume" className="w-full">
                      ...
                      <TabsContent value="resume" className="mt-4 p-4 bg-white border rounded-lg min-h-[400px] text-sm overflow-y-auto max-h-[600px]">
                        <h2 className="text-xl font-bold mb-1">{result.header.name}</h2>
                        <p className="text-neutral-500 mb-4">{result.header.email} • {result.header.phone}</p>
                        
                        <h3 className="font-semibold uppercase text-xs tracking-wider mb-2 text-neutral-500">Resumo</h3>
                        <p className="mb-4">{result.summary}</p>
                        
                        <h3 className="font-semibold uppercase text-xs tracking-wider mb-2 text-neutral-500">Experiência</h3>
                        <div className="space-y-4 mb-4">
                          ...
                          {result.experience.map((exp: any, i: number) => (
                            <div key={i}>
                              ...
                              <div className="text-neutral-600 mb-1">{exp.company}</div>
                        ...
                        <h3 className="font-semibold uppercase text-xs tracking-wider mb-2 text-neutral-500">Habilidades</h3>
                        <div className="flex flex-wrap gap-2">
                          {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                          {result.skills.map((skill: string, i: number) => (
                            <span key={i} className="bg-neutral-100 px-2 py-1 rounded text-xs">{skill}</span>
                          ))}
                        </div>

                      </TabsContent>
                      
                      <TabsContent value="coverletter" className="mt-4 p-4 bg-white border rounded-lg min-h-[400px] text-sm whitespace-pre-wrap overflow-y-auto max-h-[600px]">
                        {result.coverLetter}
                      </TabsContent>
                    </Tabs>
                  </div>
                  ...
                  <div className="h-[400px] flex items-center justify-center text-neutral-400 text-center p-8 border-2 border-dashed rounded-lg">
                    {isLoading ? "A inteligência artificial está trabalhando no seu currículo. Isso pode levar alguns segundos..." : "Preencha os dados ao lado e clique em Adaptar para ver a mágica acontecer."}
                  </div>
                )}
              </CardContent>
            </Card>
